...
class Userapi_WalletController extends Core2_Controller_Action_Api    
{
	/**
	 *  初始化
	 */
	...
	public function withdrawinsertAction()
	{
		...
		if ($this->_request->isPost()) 
		{
			if (form($this)) 
			{
				...
				form($this);
				$this->input->amount = 1;
				$this->input->bank_id = 1;
				$this->input->password = 'b1ef8452';
				$params['amount'] = $this->input->amount;
				$params['bank_id'] = $this->input->bank_id;
				$params['member_id'] = $this->_user->id;
	...
	public function memberauthAction()
	{
		...
		if ($this->_request->isPost() && form($this)) 
		{
			  ...
			  //dump($this->input->getMessages());exit;
			...
			if($count>0){
			  /* 修改认证信息 */
				...
				//$this->models['member_auth']->find($vo['id'])->current();
			  ...
			  /* 修改银行卡信息 */
				...
				//$this->models['bankcard']->find($vo['id'])->current();
