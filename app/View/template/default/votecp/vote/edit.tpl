{include file='public/admincp/header.tpl'}{include file="public/admincp/siderbar.tpl"}
	...
	<div class="page-content-wrapper">
		<div class="page-content" style="min-height:812px">
			<!--标题栏-->
			<h3 class="page-title">编辑投票 <small>编辑一个投票活动</small></h3>
			<!--路径导航-->
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li><a href="/votecp/vote/edit?id={$params.id}">编辑投票</a></li>
				</ul>
			</div>
			<!-- 错误提醒 -->
				{if $error->hasError()}
							...
							<div class="tab-content">
								<div class="form-body">
									<div class="form-group">
										...
										<div class="col-md-4"><input type="text" class="form-control" name="votename" placeholder="请输入投票活动名称" value="{$editList[0].vote_name}"></div>
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">上传横幅 <span class="required" aria-required="true">*</span></label>
										<div class="col-md-4"><input data-toggle="modal" data-target="#image_uploader" name="image" class="form-control" placeholder="点击上传图片" value="{$editList[0].image}" ></div>
									</div>
									...
									<div class="form-group" >
										...
										<div class="col-md-5">
											<textarea id="detail" name="detail" class="form-control detail" >{html content=$editList[0].rule}</textarea>
										</div>
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">活动简介</label>
										<div class="col-md-5">
											<textarea id="intro" name="intro" class="form-control">{html content=$editList[0].intro}</textarea>
										</div>
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">活动按钮名</label>
										<div class="col-md-5">
											<textarea id="vote_btn" name="vote_btn" class="form-control">{html content=$editList[0].vote_btn}</textarea>
										</div>
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">活动获奖情况</label>
										<div class="col-md-5">
											<textarea id="awards" name="awards" class="form-control detail">{html content=$editList[0].awards}</textarea>
										</div>
									</div>		
									<div class="form-group">
										...
										<div class="col-md-4">
											<div data-date-format="yyyy-mm-dd" class="input-group input-large date-picker input-daterange">
												<input type="text" name="signup_begin_time" class="form-control" placeholder="开始时间" value="{$editList[0].start_time}">
												<span class="input-group-addon">至</span>
												<input type="text" name="signup_end_time" class="form-control"  placeholder="结束时间" value="{$editList[0].end_time}">
											</div>
										</div>
									</div>
									<div class="form-group">
										...
										<div class="col-md-4">
											<div data-date-format="yyyy-mm-dd" class="input-group input-large date-picker input-daterange">
												<input type="text" name="vote_begin_time" class="form-control" placeholder="开始时间" value="{$editList[0].vstart_time}">
												<span class="input-group-addon">至</span>
												
												<input type="text" name="vote_end_time" class="form-control" placeholder="结束时间" value="{$editList[0].vend_time}">
											</div>
										</div>
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">每人总投票数</label>
										<div class="col-md-4"><input type="text" class="form-control" name="all_vote" placeholder="每人总投票数" value="{$editList[0].limit_sumnum}">
										<span class="help-block">每个粉丝总的投票数，0表示不限制，默认为0</span></div>
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">每天投票数</label>
										<div class="col-md-4"><input type="text" class="form-control" name="day_vote" placeholder="每天投票数" value="{$editList[0].limit_daynum}">
										<span class="help-block">每个粉丝每天的投票数，0表示不限制，默认为0</span></div>
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">1对1每天投票数</label>
										<div class="col-md-4"><input type="text" class="form-control" name="one_to_vote" placeholder="1对1每天投票数" value="{$editList[0].limit_dayonenum}">
										<span class="help-block">每个粉丝每天对单个参赛对象的投票数，0表示不限制，默认为0</span></div>
									</div>
						           <!--  <div class="form-group">
										<label class="control-label col-md-2">是否需要关注</label>
										<div class="col-md-4"><input type="text" class="form-control" name="subscribe" placeholder="是否需要关注" value="{$editList[0].subscribe}">
										<span class="help-block">1表示需要，0表示不需要，默认为0</span></div>
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">是否可以报名</label>
										<div class="col-md-4"><input type="text" class="form-control" name="allow_post" placeholder="是否可以报名" value="{$editList[0].allow_post}">
										<span class="help-block">1表示可以，0表示不可以，默认为0</span></div>
									</div> -->
									...
									<div class="form-group">
										<label class="control-label col-md-2">是否需要关注</label>
										<input type="radio" name="subscribe" value="1" {if $editList[0].subscribe==1} checked="checked" {/if} />需要 
                                        <input type="radio" name="subscribe" value="0" {if $editList[0].subscribe==0} checked="checked" {/if} />不需要 
									...
									<div class="form-group">
										...
										<input type="radio" name="allow_post" value="1" {if $editList[0].allow_post==1} checked="checked" {/if} />可以
                                        <input type="radio" name="allow_post" value="0" {if $editList[0].allow_post==0} checked="checked" {/if}/>不可以
										<!-- <span class="help-block">1表示可以，0表示不可以，默认为0</span></div> -->
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">选手是否需要审核</label>
										<!-- <div class="col-md-4"><input type="text" class="form-control" name="allow_post" placeholder="是否可以报名" value="0"> -->
										<input type="radio" name="player_auth" value="1" {if $editList[0].player_auth==1} checked="checked" {/if} />需要 
                                        <input type="radio" name="player_auth" value="0" {if $editList[0].player_auth==0} checked="checked" {/if}/>不需要 
									...
									<div class="form-group">
										...
										<!-- <div class="col-md-4"><input type="text" class="form-control" name="allow_post" placeholder="是否可以报名" value="0"> -->
										<input type="radio" name="comment_auth" value="1" {if $editList[0].comment_auth==1} checked="checked" {/if} />需要 
                                        <input type="radio" name="comment_auth" value="0" {if $editList[0].comment_auth==0} checked="checked" {/if}/>不需要 
	
										<!-- <span class="help-block">1表示可以，0表示不可以，默认为0</span></div> -->
									</div>
									<div class="form-group">
										<label class="control-label col-md-2">投票是否需要24小时限制</label>
										<input type="radio" name="day_auth" value="1" {if $editList[0].day_auth==1} checked="checked" {/if}  />需要 
                                        <input type="radio" name="day_auth" value="0" {if $editList[0].day_auth==0} checked="checked" {/if}  />不需要 
									</div>
									<div class="form-group">
										...
										<div class="col-md-2">
											<div class="input-group">
												...
												<input type="text" class="form-control" name="rank_num" placeholder="显示前多少名" value="{$editList[0].rank_num}">
