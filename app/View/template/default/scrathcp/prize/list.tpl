		...
		<div class="page-content" style="min-height:812px">
			...
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li><a href="/scrathcp/prize/list?scrath_id={$params.scrath_id}">刮卡记录</a></li>
				</ul>
			</div>
			<!--搜索栏-->
			<form action="/scrathcp/prize/list" id="search" name="search" method="get">
				<div class="table-responsive">
					<table class="table table-bordered">
						<thead>
							<tr role="row">
								<th class="sorting_asc" width="180px">微信名</th>
								<th class="sorting_asc" width="180px">商家</th>
						...
						<tbody>
							<tr>
								...
								<td>
									<input type="search" class="form-control" placeholder="商家" name="to_member_name" aria-controls="sample_1" value="{$params.to_member_name}">
								</td>
								<td>
...
<!-- 发货表单 -->
<div class="modal fade" id="sendOutBox" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<form action="/scrathcp/prize/ajax?op=deliver" method="post" name="sendOut" id="sendOut" class="form-horizontal">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
			  <div class="modal-header">
			    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			    <h4 class="modal-title" id="myModalLabel">发放奖品</h4>
			  </div>
...
$('.delete').click(function(){
	...
	if (confirm('确定删除？')){
		$.post('/scrathcp/prize/ajax?op=delete',{id:id},function(data){
			if (data.errno == 0){
				tr.remove();
				return;
			}else{
...
$('.send_out').click(function(){
	var id = $(this).closest('tr').data('id'),
		data=$(this).parent().data('config');
	$("#consignee").val(data.consignee);
// 	$("#invoice_number").val(data.invoice_number);
	$("#phone").val(data.phone);
	$("#address").val(data.address);
	$("#note").val(data.note);
	$('#sendOutBox input#id').val(data.id); 
	$('#sendOutBox').modal('show');
});
//查看
