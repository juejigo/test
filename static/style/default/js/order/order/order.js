...
var rightDom=$("#right"),
    rightDomTop = rightDom.offset().top;
...
function addPerson(id,callback){
  $.post('../person.php',{id:id},function(data){
    if(data.errno==0){
        callback(data.tourist[0]);
    }else{
        layer.msg('勾选失败');
    }
  },'json')
...
$("#addTourist").submit(function(e){
  ...
  $.post(url,_this.serialize(),function(data){
    if(data.errno==0){
      ...
      $(".order_person_list ul").append('<li data-id="'+data.id+'"><i class="check"></i>'+tourist_name.val()+'</li>')
      layer.msg('出行人添加成功');
...
function pay(){
  ...
  $(".pay_btn").attr("disabled","disabled").html("正在提交...");
  $.post('../calendar.php',{
    order:order,
    type:type
  },function(data){
    if(data.errno==0){
      ...
      $(".pay_btn").removeAttr("disabled").text("确认支付");
