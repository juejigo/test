 var fun={
	init:function(callback){
		...
		if(!data){
			var $canvas=$("<canvas/>"),		//创建画布ID
				...
				cWidth=$("#getAfter").width(),		//画布宽度
				cHeight=$("#getAfter").height(),	//画布高度
				pos = $(this).offset(),		//相对偏移量
				size=30,		//画笔大小
				completeRatio=0.01,		//完成度
				parts = [],
	...
	touchEnd:function(event){
      ...
      //     // var tx = ta[n].pageX - data.posX,
      //     //   ty = ta[n].pageY - data.posY;
      //     // fun.evaluatePoint( data, tx, ty , event.type );		//进行擦除率判断
	...
	//计算擦除率
	evaluatePoint:function(data,tx,ty,type){
    fun.clear(data);
		...
		// 	data.parts[p] = 0;
		// 	if (!data.complete) {
		// 		if ( data.ratio/data.numParts >= data.completeRatio && type=='touchend') {
		// 			data.complete = true;
		// 			fun.clear(data);
		// 			if(data.completeCall!= null){
		// 				data.completeCall();
	...
	clear:function(data){
		...
		data.canvas.animate({"opacity":0,"z-index":-11},300);
		var n = data.numParts;
