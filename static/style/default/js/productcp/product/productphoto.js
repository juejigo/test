...
var uploader = new plupload.Uploader({
    ...
    url : "/productcp/image/imgupload",		//上传地址
    multipart_params:{
    	product_id:$("#prodId").val()      //商品ID
    },
    filters : {
    ...
    init: {
        PostInit: function(){
			...
			imgBox.on("click",".handleDefault",function(){
				...
				$.post("/productcp/image/ajax?op=setdefault",{image_id:id,product_id:productId}).done(function(data){
					//var data=eval("(" + data + ")");
					if(data.errno == 1){
			...
			imgBox.on("click",".handleDelImg",function(){
				var _this=$(this).parents("li"), 
					id=_this.data("imgid");
				$.post("/productcp/image/ajax?op=delete",{image_id:id,product_id:productId}).done(function(data){
					if(data.errno == 1){
        ...
        FileUploaded: function(up, file, data) {
            ...
            if (response.erron==1) {
		        ...
		        //添加到图片框
		        var str='';
		        str+='<li class="handle_img" data-imgid="'+response.imgid+'">';
...
$("#handleImgBox").sortable({
    ...
    update:function(){
    	...
    	$.post('/productcp/image/sortimg',{order:order});
    }
});
